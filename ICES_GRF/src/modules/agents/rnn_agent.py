...
class RNNAgent(nn.Module):
    ...
    def forward(self, inputs, hidden_state):
        ...
        if len(input_shape) == 2:
            ...
            gru_out = gru_out.reshape(-1, self.args.n_agents, self.args.rnn_hidden_dim)
            local_q = torch.stack(
                [mlp(gru_out[:, id, :]) for id, mlp in enumerate(self.mlp)], dim=1
            )
            local_q = local_q.reshape(-1, self.args.n_actions)
            ...
            local_q = torch.stack(
                [
                    mlp(gru_out_local[:, id].reshape(-1, gru_out_local.shape[-1]))
                    for id, mlp in enumerate(self.mlp)
                ],
                dim=1,
            )
